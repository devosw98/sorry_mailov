document.addEventListener('DOMContentLoaded', (event) => {
    ...
    noBtn.addEventListener('click', () => {
        ...
        yesBtn.style.width = currentWidth * 1.3 + 'px';
        yesBtn.style.height = currentHeight * 1.3 + 'px';

        const currentFontSize = window.getComputedStyle(yesSpn).fontSize;
        yesSpn.style.fontSize = parseFloat(currentFontSize) * 1.3 + 'px';

        const currentBorderRadius = window.getComputedStyle(yesBtn).borderRadius;
        yesBtn.style.borderRadius = parseFloat(currentBorderRadius) * 1.3 + 'px';
    }),
